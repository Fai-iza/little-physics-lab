...
public class Scene2_2Controller implements Initializable {
   
    @FXML private RadioButton chris;
@FXML private RadioButton newton;
@FXML private RadioButton max;
...
public static int points2=0;
    ...
    if (this.q1.getSelectedToggle().equals(this.newton))
        points2=points2+1;
    if (this.q2.getSelectedToggle().equals(this.q2true))
        points2=points2+1;
    if (this.q3.getSelectedToggle().equals(this.q3false))
        points2=points2+1;
        ...
        points2=points2+1;
            ...
            for (score s : pList) {
                if (s.getUsername().equals(loginController.username_2)) {
                    s.setNewtons_third_low(points2);
    ...
    Parent tableViewParent =  FXMLLoader.load(getClass().getResource("lvl2_star.fxml"));
    Scene tableViewScene  = new Scene(tableViewParent);
    //his line gela the Stage informal.ion
    Stage window =(Stage)((Node)event.getSource()).getScene().getWindow();
    window.setScene (tableViewScene);
